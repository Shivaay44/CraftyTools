...
export const MetaTagGeneratorWidget: React.FC = () => {
  const [title, setTitle] = useState<string>('Toolchemy — 100% Free Online Browser Utilities');
  const [description, setDescription] = useState<string>(
    'Free online tools for developers, creators, and professionals. Fast, private, zero-API client-side tools.'
  );
  const [url, setUrl] = useState<string>('https://toolchemy.vercel.app');
  ...
  const [siteName, setSiteName] = useState<string>('Toolchemy');
  const [twitterHandle, setTwitterHandle] = useState<string>('@toolchemy');
